...
impl<T> Grid<T> {
    pub fn filled_with(width: usize, height: usize, elem: T) -> Self where T: Clone {
        let data = std::iter::repeat(elem).take(width * height).collect::<Vec<_>>();
        Self {
            data,
    ...
    pub fn filled_with_fn(width: usize, height: usize, f: impl FnMut() -> T) -> Self {
        let data = std::iter::repeat_with(f).take(width * height).collect::<Vec<_>>();
        Self {
            data,
            dimensions: (width, height),
        }
    ...
    pub fn from_iter(width: usize, height: usize, iter: impl Iterator<Item = T>) -> Option<Self> {
        ...
        if data.len() < width * height {
            ...
            Some(Self {data, dimensions: (width, height)})
        }
    }
    ...
    pub unsafe fn get_unchecked(&self, x: usize, y: usize) -> T where T: Clone {
        let index = self.index_from_xy(x, y);
        self.data.get_unchecked(index).clone()
    }

    pub fn get(&self, x: usize, y: usize) -> T where T: Clone {
        assert!(x < self.dimensions.0 && y < self.dimensions.1);
        unsafe { self.get_unchecked(x, y) }
    }

    pub fn into_row_major_iter(self) -> impl Iterator<Item = (usize, usize, T)> {
        self.data.into_iter().enumerate().map(move |(i, t)| {
            (i % self.dimensions.0, i / self.dimensions.0, t)
        })
    }
